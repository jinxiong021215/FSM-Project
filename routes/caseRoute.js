...
router.post('/payments/complete', verifyJWT('admin', 'company', 'customer'), (req, res) => {
  ...
  // Ownership and status checks before proceeding (especially for customers)
  const authUser = req.user; // { userId, role, name }
  const ownershipSql = `
    SELECT c.customer_id, c.status AS case_status
    FROM Bill b
    JOIN Cases c ON b.case_id = c.case_id
    WHERE b.bill_id = ?
  `;
  db.query(ownershipSql, [bill_id], (ownErr, ownRows) => {
    if (ownErr) {
      console.error('Ownership check error:', ownErr);
      return res.json({ success: false, message: ownErr.message || 'DB error' });
    }
    if (!ownRows || !ownRows.length) {
      return res.status(404).json({ success: false, message: 'Bill not found' });
    }

    const ownerCustomerId = ownRows[0].customer_id;
    const caseStatus = ownRows[0].case_status;

    if (authUser.role === 'customer') {
      if (String(ownerCustomerId) !== String(authUser.userId)) {
        return res.status(403).json({ success: false, message: 'Forbidden: Not your bill' });
      }
      // Optional: ensure customer can only pay when case is COMPLETE
      if (String(caseStatus).toUpperCase() !== 'COMPLETE') {
        return res.status(400).json({ success: false, message: 'Payment not available until case is COMPLETE' });
      }
    }
