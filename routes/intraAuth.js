...
// Admin Register (protected: admin, company)
router.post('/register-admin', require('./auth').verifyJWT('admin', 'company'), (req, res) => {
  ...
  const isCompany = req.user && req.user.role === 'company' && req.user.companyId;
  const insertBase = 'INSERT INTO Admin (name, email, phone, password_hash, status) VALUES (?, ?, ?, ?, ?)';
  const insertWithCompany = 'INSERT INTO Admin (name, email, phone, password_hash, status, company_id) VALUES (?, ?, ?, ?, ?, ?)';

  const tryInsert = (withCompany) => new Promise((resolve) => {
    const sql = withCompany ? insertWithCompany : insertBase;
    const params = withCompany ? [name, email, phone, hashedPassword, 'ACTIVE', req.user.companyId] : [name, email, phone, hashedPassword, 'ACTIVE'];
    db.query(sql, params, (error, results) => resolve({ error, results }));
  });

  (async () => {
    let resp = await tryInsert(isCompany);
    if (resp.error && resp.error.code === 'ER_BAD_FIELD_ERROR' && isCompany) {
      // Fallback when company_id column does not exist
      resp = await tryInsert(false);
    }
...
// Technician Register (protected: admin, company)
router.post('/register-technician', require('./auth').verifyJWT('admin', 'company'), (req, res) => {
  const { name, email, phone, password } = req.body;
  if (!name || !email || !phone || !password) {
    return res.status(400).json({ success: false, message: 'Missing required fields.' });
  }
  ...
  const hashedPassword = bcrypt.hashSync(password, 10);
  const isCompany = req.user && req.user.role === 'company' && req.user.companyId;
  const insertBase = 'INSERT INTO Technician (name, email, phone, password_hash, status) VALUES (?, ?, ?, ?, ?)';
  const insertWithCompany = 'INSERT INTO Technician (name, email, phone, password_hash, status, company_id) VALUES (?, ?, ?, ?, ?, ?)';

  const tryInsert = (withCompany) => new Promise((resolve) => {
    const sql = withCompany ? insertWithCompany : insertBase;
    const params = withCompany ? [name, email, phone, hashedPassword, 'OFF_DUTY', req.user.companyId] : [name, email, phone, hashedPassword, 'OFF_DUTY'];
    db.query(sql, params, (error, results) => resolve({ error, results }));
  });
  ...
  (async () => {
    ...
    if (error) {
      if (error.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: 'Email or phone already in use.' });
      console.error('Register technician failed:', error);
      return res.status(500).json({ success: false, message: 'Registration failed.' });
    }
    return res.json({ success: true });
  })();
...
// Update Admin status (protected: admin, company)
router.put('/employees/admins/:id/status', verifyJWT('admin', 'company'), (req, res) => {
...
// Update Technician status (protected: admin, company, or technician self)
router.put('/employees/technicians/:id/status', verifyJWT('admin', 'company', 'technician'), (req, res) => {
  const userId = req.params.id;
  const { status } = req.body;
  // If the caller is a technician, only allow changing their own status
  if (req.user && req.user.role === 'technician' && String(req.user.userId) !== String(userId)) {
    return res.status(403).json({ success: false, message: 'Forbidden: cannot change another technician\'s status.' });
  }
...
// Reset Admin password (protected: admin, company)
router.put('/employees/admins/:id/password', verifyJWT('admin', 'company'), (req, res) => {
  const userId = req.params.id;
...
// Reset Technician password (protected: admin, company)
router.put('/employees/technicians/:id/password', verifyJWT('admin', 'company'), (req, res) => {
  const userId = req.params.id;
  const { new_password } = req.body;
  if (!new_password || String(new_password).length < 6) {
    return res.status(400).json({ success: false, message: 'Password must be at least 6 characters.' });
...
// Update Admin profile fields (protected: admin, company)
router.put('/employees/admins/:id', verifyJWT('admin', 'company'), (req, res) => {
  const userId = req.params.id;
...
// Update Technician profile fields (protected: admin, company)
router.put('/employees/technicians/:id', verifyJWT('admin', 'company'), (req, res) => {
  const userId = req.params.id;
  const { name, email, phone } = req.body || {};
  const fields = [];
  const params = [];
