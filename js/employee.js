...
function setupAddEmployee() {
  ...
  document.getElementById('addEmployeeForm').onsubmit = async function(e) {
    ...
    try {
      ...
      let data = {};
      try { data = await res.json(); } catch { data = { success: false, message: 'Unexpected response' }; }
      if (res.ok && data.success) {
        bootstrap.Modal.getInstance(document.getElementById('addEmployeeModal')).hide();
        await fetchEmployeeData();
        renderEmployeeTables();
      } else {
        const msg = (data && data.message) ? data.message : `Add failed (HTTP ${status}).`;
        alert(msg);
      }
    } catch (err) {
      console.error('Add employee error:', err);
      alert('Add failed: network or server error.');
    }
  };
}
