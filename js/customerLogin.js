...
function handleRegister(event) {
  ...
  .then((data) => {
    ...
    if (data.success) {
      showFeedback(registerFeedback, 'Registration successful! You can log in now.', true);
      try {
        const modalEl = document.getElementById('registerSuccessModal');
        if (modalEl) {
          const modal = new bootstrap.Modal(modalEl);
          const msgEl = document.getElementById('register-success-text');
          if (msgEl) msgEl.textContent = 'Your account has been created successfully.';
          const goBtn = document.getElementById('btnGoLogin');
          if (goBtn) {
            goBtn.onclick = () => {
              modal.hide();
              // Switch to Login tab when user chooses
              try { showLogin(new Event('click')); } catch { /* no-op */ }
            };
          }
          modal.show();
        }
      } catch (e) { console.warn('Register success modal error:', e); }
    } else {
